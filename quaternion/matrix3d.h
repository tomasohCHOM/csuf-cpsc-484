...
public:
  ...
  static void run_tests() {
    ...
    std::cout << "test copy constructor -- program will crash at end if this "
                 "is incorrect\n";
    matrix3D acopy(a); // copy constructor
    acopy.show();

    std::cout << "test copy constructor using =  -- program will crash at end "
                 "if this is incorrect\n";
    matrix3D a2copy = a; // copy constructor
    a2copy.show();

    std::cout << "test assignment operator  -- program will crash at end if "
                 "this is incorrect\n";
    matrix3D bcopy;
...
template <typename T>
matrix3d<T>::matrix3d(const std::string &name, int dims)
...
template <typename T> matrix3d<T> matrix3d<T>::cofactor() const {
  const matrix3d<T> &m = *this;
  const matrix3d<T> &n = m.minors();
  matrix3d<T> res(m.name() + "_Cofactors", m.dims_,
...
template <typename T> matrix3d<T> matrix3d<T>::adjoint() const {
  const matrix3d<T> &m = *this;
  const matrix3d<T> &n = m.cofactor();
  matrix3d<T> res(m.name() + "_Adjoint", m.dims_,
                  {n(0, 0), n(1, 0), n(2, 0), n(0, 1), n(1, 1), n(2, 1),
                   n(0, 2), n(1, 2), n(2, 2)});
  return res;
}
template <typename T> matrix3d<T> matrix3d<T>::inverse() const {
  const matrix3d<T> &m = *this;
  matrix3d<T> adj = m.adjoint();
  adj /= m.determinant();
  matrix3d<T> res(m.name() + "_Inverse", m.dims_,
                  {adj(0, 0), adj(0, 1), adj(0, 2), adj(1, 0), adj(1, 1),
                   adj(1, 2), adj(2, 0), adj(2, 1), adj(2, 2)});
  return res;
}
//=================================================================================================
template <typename T> matrix3d<T> matrix3d<T>::identity(int dims) {
  return matrix3D("Identity", 3, {1, 0, 0, 0, 1, 0, 0, 0, 1});
}
