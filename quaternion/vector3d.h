...
public:
  ...
  T x() const { return data_[0]; } // read-only
  T y() const { return data_[1]; }
  T z() const { return data_[2]; }

  T &x() { return data_[0]; } // read-write
  T &y() { return data_[1]; }
  T &z() { return data_[2]; }
  ...
  friend vector3d<T> operator+(const vector3d<T> &v, T k) {
    return vector3d<T>(v.name_ + "+" + std::to_string(k), v.dims_,
                       {v[0] + k, v[1] + k, v[2] + k, 0});
  }
  ...
  friend bool operator==(const vector3d<T> &u, const vector3d<T> &v) {
    check_equal_dims(u, v);
    return /* u.name_ == v.name_ && */ u[0] == v[0] && u[1] == v[1] &&
           u[2] == v[2];
  }
  friend bool operator!=(const vector3d<T> &u, const vector3d<T> &v) {
    return !(u == v);
...
template <typename T> double vector3d<T>::angle(const vector3d<T> &v) const {
  ...
  // then...    a.dot(b) / (a.mag() * b.mag()) = cos(theta)
  // and...     theta = arccos(a.dot(b) / (a.mag() * b.mag()))
